...
static void (*mNewControllerDataCallback)(SControllerData*) = NULL;
...
bool HeaterTemperatureController_setPower(u16 power)
{
    ...
    if (result)
    {
        Logger_debug("%s: Set power: %u (%.2f /%).", getLoggerPrefix(), power, ( ( (float) power ) / MAX_HEATER_POWER_F ) * 100.0F );
    }
    else
    {
...
void controllerAlgorithm(void const* arg)
{
    ...
    SControllerData controllerData;
    ...
    if (mHeaterControlValue != controllerData.CV)
    {
    ...
    {
        (*mNewControllerDataCallback)(&controllerData);
    }
